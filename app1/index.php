...
$app->get('/consultaDatos', function ($request, $response, $args) {  //Defino los servicios
	try{
		...
		$sth = $db->prepare("SELECT datId, datNombre, datApellido, datEdad, datDeporte, datImagen, datDescripcion from datos");//Consulta
...
$app->post('/createDatos', function ($request, $response, $args) {  //Defino los servicios  $app->post('/updateVeces', function ($request, $response)
	try{
		//$data = $request->getParsedBody(); //Recupero los datos  getParams()
		$json = $request->getBody();
		$data = json_decode($json, true);
		$db =  getDB(); //Carga los datos
		$sth = $db->prepare("INSERT INTO datos (datNombre, datApellido, datEdad, datDeporte, datImagen, datDescripcion) VALUES (?,?,?,?,?,?)");//Insertamos información
	
$sth->execute(array($data['datNombre'],$data['datApellido'],$data['datEdad'],$data['datDeporte'],$data['datImagen'],$data['datDescripcion'])); //Sustituimos y ejecutamos la consulta
...
// Hacemos la consulta para eliminar datos ->
...
$app->post('/removeDatos', function ($request, $response, $args) {  //Defino los servicios  $app->post('/updateVeces', function ($request, $response)
	try{
		...
		$data = json_decode($json, true);
		$db =  getDB(); //Carga los datos
		$sth = $db->prepare("DELETE FROM datos WHERE datId = ?");//Insertamos información
...
$app->post('/updateDatos', function ($request, $response, $args) {  //Defino los servicios  $app->post('/updateVeces', function ($request, $response)
	try{
		...
		$data = json_decode($json, true);
		$db =  getDB(); //Carga los datos
		$sth = $db->prepare("UPDATE datos SET datNombre = ?, datApellido = ?, datEdad = ?, datDeporte = ?, datImagen = ?, datDescripcion = ? WHERE datId = ?");//Insertamos información
	
$sth->execute(array($data['datNombre'],$data['datApellido'],$data['datEdad'],$data['datDeporte'],$data['datImagen'],$data['datId'], $data['datDescripcion'])); //Sustituimos y ejecutamos la consulta
		$response->getBody()->write('{"error":"ok"}'); //Cuando la conexión halla terminado
